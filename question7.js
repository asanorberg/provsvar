...
dataPromise.then((data) => {
  ...
  const filterId = data.filter((item) => item.userId == 1);
  ...
  data.forEach((post) => {
    ...
    postEl.innerHTML = `<div class = "posts-div"><h4>${post.title}</h4></div>`;
  ...
  dataPromise.then((data2) => {
    ...
    filterId.forEach((comment) => {
      const commentEl = document.createElement("div");
      commentEl.innerHTML = `<div class = "comments-div"><h4>${comment.body}</h4></div>`;
